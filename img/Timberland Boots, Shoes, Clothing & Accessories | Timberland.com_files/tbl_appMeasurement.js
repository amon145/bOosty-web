//var s_account="vfctimberlandstag"
var s=s_gi(s_account)

//s_getLoadTime(); // to record page load times - initial 
	/************************** CONFIG SECTION **************************/
	...
	s.linkInternalFilters="javascript:,timberland.com"
	...
	s.trackingServer="vfcorp.d1.sc.omtrdc.net"
	...
	s.prop10=s.getPreviousValue(s.pageName,'gpv','');
	if(s.prop10){
	   s.eVar23="D=c10";
	}
    ...
    s.prop12 = s.eVar21 = "D=g";//Capture full URL
    
        
        
    /* Time Parting */
	var timeVar = s.getTimeParting('n','-5');
	...
	s.eVar2=s.getNewRepeat(30,'s_getNewRepeat');
	...
	s.prop11 = load_time;
	s.events=s.apl(s.events,'event20='+load_time,',',1);
	...
	if(s.events && s.events.indexOf('scAdd')>-1)
	{
		...
		s.eVar9=s.pageName;//to capture the add-to-cart location
	}
    ...
    if(!s.campaign)
	{
		s.campaign=s.Util.getQueryParam('camp');
	...
	if(s.prop8)
	{
		...
		if(s.prop5&&(s.prop9=='0'||s.prop9=='zero'))
		{
			s.prop9='zero';
    ...
    s.refDomain=a.hostname;
        ...
        if(!s.eVar19){
            if(s.campaign){s.eVar19="campaign"}  
            else if(s.eVar3){s.eVar19="Internal Campaign"} 
            else if(s.eVar8){s.eVar19="internal search"}
            else if(s.eVar15){s.eVar19="browse: category"}
            else if(document.referrer==""){s.eVar19="direct or bookmarked"}
            else if(s.refDomain.indexOf("timberland.com")==-1){s.eVar19="external site"} 
		}
